...
function getUsageHelp(commandName) {
    function createSample(target) {
        return commandName + ' *' + target + '* Message Here';
    }

    var text = 'Expected usage: \n' +
        commandName + ' help -- Displays help message.\n' +
        createSample('#suggestions') + ' -- Sends to private channel for Department Head.\n' +
		'\n' +
...
function getFullHelp(commandName) {
    var text =
        'Allows to send suggestion messages to the Department Head.\n' +
        'The most convenient and safe way is to open up a conversation with slackbot in Slack and type the command there, so that nobody detects that you are typing and you don\'t accidentally reveal yourself by typing an invalid command.\n' +
        '\n' +
